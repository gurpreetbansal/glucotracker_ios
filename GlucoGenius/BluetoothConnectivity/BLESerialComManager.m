...
-(NSData *)readDataFromPort:(BLEPort *)port withLength:(int)length{
    if (port!=nil) {
        if (port.readBuffer) {
            int realLen = (length<=port.readBuffer.length)?length:port.readBuffer.length;
...
-(resultCodeType)clearReadBufferInPort:(BLEPort *)port{
    ...
    if (port.readBuffer && port.readBuffer.length != 0) {
        NSLog(@"In clearReadBufferInPort : cleared length = %d",port.readBuffer.length);
...
-(void)dataCallBack:(NSData *)data fromPort:(BLEPort *)port{
    ...
    if ([self.delegate respondsToSelector:@selector(bleSerialComManager:didDataReceivedOnPort:withLength:)]) {
            [self.delegate bleSerialComManager:self didDataReceivedOnPort:port withLength:data.length];
    }
...
-(NSString *)getStateString:(CBCentralManagerState)state{
    switch (state) {
...
-(void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral{
   BLEPort *port = [self getPortByPeripheral:peripheral];
    ...
    NSLog(@"didConnect with %@ name %s",peripheral.identifier.UUIDString,[peripheral.name UTF8String]);
...
-(void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error{
    ...
    for (CBCharacteristic *c in service.characteristics) {
        NSLog(@"=> characteristic <%s> 0x%2x",[self CBUUIDToString:c.UUID],c.properties);
    }
